...
const THEMES: Record<BuiltinTheme, Record<string, SegmentStyle>> = {
  'pastel-rainbow': {
...
function unescapeTomlString(s: string): string {
  ...
  return s.replace(/\\(["\\ntr]|u[0-9a-fA-F]{4})/g, (_, esc: string) => {
    if (esc === '"') return '"';
    if (esc === '\\') return '\\';
    if (esc === 'n') return '\n';
    if (esc === 't') return '\t';
  ...
  if (raw.startsWith('[') && raw.endsWith(']')) {
    ...
    while (i < inner.length) {
      ...
      if (inner[i] === '"' || inner[i] === "'") {
        ...
        if (token) items.push(parseTomlValue(token));
...
export async function loadConfig(): Promise<ClaudeMenuConfig> {
  try {
...
function mergeConfig(
  ...
  if (overrides.layout && typeof overrides.layout === 'object') {
    const lo = overrides.layout as Record<string, unknown>;
    if (lo.mode) config.layout.mode = lo.mode as 'expanded' | 'compact';
    if (Array.isArray(lo.segments)) config.layout.segments = lo.segments as typeof config.layout.segments;
  }
...
export function resolveSegmentStyle(
  ...
  // Then built-in theme
  const builtinTheme = THEMES[theme.name as BuiltinTheme];
  const base = builtinTheme?.[segmentName] ?? { fg: '#ffffff', bg: '#555555' };

  return { ...base, ...override };
}

export function getThemeSeparator(theme: ThemeConfig): string {
